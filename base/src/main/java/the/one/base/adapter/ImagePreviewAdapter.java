...
/**
 * @author https://github.com/SherlockGougou/BigImageViewPager
 */
public class ImagePreviewAdapter extends PagerAdapter {
    ...
    public ImagePreviewAdapter(Activity activity, @NonNull List<ImageInfo> imageInfo,OnImageListener listener) {
        super();
    ...
    public Object instantiateItem(@NonNull ViewGroup container, final int position) {
        ...
        if (cacheFile != null && cacheFile.exists()) {
            ...
            loadImage(url, imageView, imageGif, progressBar, 1);
        }

        setListener(url,position,imageView, imageGif);
        container.addView(convertView);
        return convertView;
    }

    private void setListener(String path,int position,View... views){
        if(null != listener){
            for (View view:views){
                view.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        listener.onClickListener(path,position,v);
                    }
                });
                view.setOnLongClickListener(new View.OnLongClickListener() {
                    @Override
                    public boolean onLongClick(View v) {
                        return listener.onLongClickListener(path,position,v);
                    }
                });
            }
        }
    ...
    private void loadImage(String url, SubsamplingScaleImageViewDragClose imageView, ImageView imageGif, ProgressBar progressBar, final int count) {
        Glide.with(activity).downloadOnly().load(url).addListener(new RequestListener<File>() {
            ...
            public boolean onLoadFailed(@Nullable GlideException e, Object model,
                ...
                if (count > 3)
                    ...
                    loadImage(url, imageView, imageGif, progressBar, count + 1);
                return true;
            }
    ...
    public void destroyItem(@NonNull ViewGroup container, int position, @NonNull Object object) {
        ...
        try {
            ImageLoader.clearMemory((AppCompatActivity) activity);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public interface OnImageListener{
       void onClickListener(String path,int position,View view);
       boolean onLongClickListener(String path,int position,View view);
    }
}